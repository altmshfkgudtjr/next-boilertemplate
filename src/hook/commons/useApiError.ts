import { axiosErrorLogFormat } from 'util/error';
import { Error_4XX, Error_5XX } from 'util/error';
...
const defaultHandler = error => {
  ...
  isDev && console.warn('[Default Error Handler] 정의되지 않은 에러가 발생했습니다.');
  return error;
};
...
const apiErrorHandler = (handlers?: any) => {
  const handleError = error => {
    ...
    const serviceCode = error.response?.data?.error?.code;
